...
import type { AddedUnitOfForm, CurrentStepProps, StepProps } from '.';
import * as S from './styles';

export function Step2({ promotion, handleChangePromotion, promotionType }: StepProps) {
  ...
  const handleAddAddedUnits = (unity: AddedUnitOfForm) =>
    handleChangePromotion('addedUnits', [...promotion.addedUnits, unity]);

  const handleRemoveAddedUnits = (id: string) =>
    handleChangePromotion(
      'addedUnits',
      promotion.addedUnits.filter((item) => item.unit_id !== id)
    );
  ...
  useEffect(() => {
    if (unitsDataFromCsv) handleChangePromotion('addedUnits', unitsDataFromCsv);
                ...
                <div className="left-content">
                  <Checkbox
                    ...
                    onChange={({ target: { checked } }) =>
                      checked && !promotion.addedUnits.find((currentUnit) => currentUnit.unit_id === unity.id)
                        ? handleAddAddedUnits(
                            Object.assign(
                              {
                                unit_id: unity.id,
          ...
          <S.SearchBoxDescriptionWrapper>
            ...
            <S.SearchBoxDescriptionButtons>
              <Popover content={helperForImportCSVContent}>
                <Button variant="outlined" htmlType="button" size="small" onClick={() => csvInputRef?.current?.click()}>
            ...
            <FormItem title="Busca" minHeight="4rem">
              ...
              <Input
                ...
                onChange={({ target: { value } }) => {
                  ...
                  if (!value) {
                    handleChangePromotion('addedUnits', promotionsBeforeFiltering);
