...
export const ComunicationReservationPage = () => {
  const dispatch = useDispatch();
  const {
    comunication: { smsMessageData, settings, loading },
  } = useSelector((state: RootType) => state);
  const { unitId } = useParams<'reservation.comunication'>();
  ...
  const handleCustomMessage = (values: SubmitValues) => {
    ...
    if (settings) {
      ...
      const messageDefault = smsMessageData['reservation'].filter((item) => item.type === values.key)[0].default;
  ...
  const customSmsPanel = () => (
    ...
    >
      <S.WrapperSms>
        ...
        <MessageEdit
          experienceType="reservation"
          type="sms"
          listMessages={smsMessageData.reservation}
          listMessagesUpdated={
            settings?.custom_sms && Object.keys(settings?.custom_sms).indexOf('reservation') >= 0
              ? settings?.custom_sms['reservation']
              : {}
          }
          defaultMessage={smsMessageData.reservation.length ? smsMessageData.reservation[0].type : ''}
    ...
    <S.Pane tab="Configurações" key={TabKeysComunication.COMMUNICATION_SETTINGS}>
      <ComunicationSettingsSection>
        <ComunicationSettingsSwitches
          ...
          whatsapp={{
            enabled: true,
            price: import.meta.env.VITE_WHATSAPP_COMMUNICATION_RESERVATION_PRICE,
          }}
        />
      </ComunicationSettingsSection>
    </S.Pane>
